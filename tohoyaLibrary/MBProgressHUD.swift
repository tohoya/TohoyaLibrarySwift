...
func MB_TEXTSIZE(_ text: String?, font: UIFont) -> CGSize {
    return (text != nil && text!.characters.count > 0) ? (text! as NSString).size(attributes: [NSFontAttributeName: font]) : CGSize.zero
}

func MB_MULTILINE_TEXTSIZE(_ text: String?, font: UIFont, maxSize: CGSize, mode: NSLineBreakMode) -> CGSize {
    return (text != nil && text!.characters.count > 0) ? (text! as NSString).boundingRect(with: maxSize, options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: [NSFontAttributeName: font], context: nil).size : CGSize.zero
}
...
//    var isFinished: Bool?
    ...
    var customView: UIView? {
        didSet {
            ...
            self.swift_performSelector(onMainThread: #selector(UIView.setNeedsLayout), with: nil, waitUntilDone: false)
            self.swift_performSelector(onMainThread: #selector(UIView.setNeedsDisplay(_:)), with: nil, waitUntilDone: false)
        }
    }
...
//        isFinished = true
    ...
    func launchExecution() {
        autoreleasepool {
            (targetForExecution as! NSObject).swift_perform(methodForExecution!, with: objectForExecution)
            self.swift_performSelector(onMainThread: #selector(MBProgressHUD.cleanUp), with: nil, waitUntilDone: false)
        }
    }
...
//            CGContextDrawRadialGradient(context, gradient, gradCenter, 0, gradCenter, gradRadius,CGGradientDrawingOptions(kCGGradientDrawsAfterEndLocation))
            context.drawRadialGradient(gradient, startCenter: gradCenter, startRadius: 0, endCenter: gradCenter, endRadius: gradRadius,options: CGGradientDrawingOptions(arrayLiteral: CGGradientDrawingOptions.drawsAfterEndLocation))
        }
...
class MBBarProgressView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        context.addArc(center: CGPoint(x: 2, y: 2), radius: radius, startAngle: radius + 2, endAngle: 2, clockwise: false)
